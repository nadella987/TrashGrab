                        ...
                        <form action="{{ route('transaksi.updateStatus', ['id' => $transaksi->id]) }}" class="col-md-2" method="POST">
                            ...
                            <div class="form-group mb-0">
                                <select class="form-control form-control-user" id="status" name="status" onchange="this.form.submit()">
                                    ...
                                    <option value="ajukan_pembatalan" {{ $transaksi->status == 'ajukan_pembatalan' ? 'selected' : '' }}>Ajukan Pembatalan</option>
                                </select>
                            </div>
                        </form>
                        @else
            ...
            <div class="card p-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        @if(auth()->user()->role == 'member')
                        <a  href="{{route('sampah.create', ['transaksi_id' => $transaksi->id]) }}" class="btn btn-primary">Tambah Sampah</a>
                        @endif 
                    </div>
                    <div class="row">
